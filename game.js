...
function drawPlayer() {
    ...
    // Щит
    if (p.shield > 0) {
        ctx.strokeStyle = `rgba(0, 255, 200, ${0.3 + Math.sin(Date.now() * 0.005) * 0.2})`;
        ctx.lineWidth = 2;
        ctx.beginPath();
...
function drawBullets() {
    for (const b of bullets) {
        if (b.isPlayer) {
            // След для пуль игрока
            for (let j = 0; j < b.trail.length; j++) {
                const alpha = (j / b.trail.length) * 0.5;
                ctx.fillStyle = `rgba(0, 255, 255, ${alpha})`;
                const s = b.width * (j / b.trail.length) * 0.7;
                ctx.fillRect(b.trail[j].x - s / 2, b.trail[j].y - s / 2, s, s);
            }
...
function init() {
    ...
    // Mouse support for joystick (for testing)
